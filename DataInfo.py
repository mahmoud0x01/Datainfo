...
def parse_args():
    ...
    parser.add_argument('-rsa','--rsa',help="RSA asymmetric crypto algorithm operations to generate just type bit lenth to get PublicKey type pub")
...
def PublicKeyCrypto(input,output,form,password):
	...
	if (rsa == 1024 or 2048 or 4096 or 8192) and not "pub":
		print B +'[+]' + W + "generating RSA keys" + "\n"
		a1 = hex(id(rsa))
		print (B + "IO" +"["+a1+"]" + " RSA algorithms" + W + "\n")
		key = RSA.generate(int(rsa))
		...
		if form == "pem" or "der":
			...
			print R + '[!]' + "Error please specify export format with -f"
			sys.exit()
		if password:
...
def main():
	sysinfo()
	if rsa and not encrypt or decrypt or encode or decode or random or hash:
		banner()
